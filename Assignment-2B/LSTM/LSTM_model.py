...
def _resolve_data_path(data_path: str | Path | None = None) -> Path:
    ...
    raise FileNotFoundError(
        f"Could not find {DEFAULT_DATA_FILE} next to LSTM_model.py or in the repo root."
    )
...
def _require_tensorflow():
    ...
    return Sequential, Input, LSTM, Dense, Dropout, EarlyStopping
...
def build_lstm_model(
    ...
    Sequential, Input, LSTM, Dense, Dropout, _ = _require_tensorflow()

    model = Sequential(
        [
            Input(shape=(1, num_features)),
            LSTM(first_units, return_sequences=True),
...
def train_lstm_model(
    ...
    model = build_lstm_model(num_features=x_train_3d.shape[2])
...
def traffic_lstm(
    ...
    return predict_lstm_with_results(results, predict_day)
...
def save_lstm_plots(
    ...
    plt.title(
        f"LSTM - Traffic Volume Prediction ({results['location']}, {results['time_column']})"
    )
